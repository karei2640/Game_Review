class Public::GamesController < ApplicationController
  ...
  def index
    ...
    @latest_games = Game.order(created_at: :desc).page(params[:latest_games_page]).per(1)
    @popular_games_all = Game.joins(:view_counts).group(:id).order('count(view_counts.id) desc').page(params[:popular_games_all_page]).per(1)
    @popular_games_monthly = Game.joins(:view_counts).where(view_counts: { created_at: 1.month.ago..Time.current }).group(:id).order('count(view_counts.id) desc').page(params[:popular_games_monthly_page]).per(1)
    @popular_games_weekly = Game.joins(:view_counts).where(view_counts: { created_at: 1.week.ago..Time.current }).group(:id).order('count(view_counts.id) desc').page(params[:popular_games_weekly_page]).per(1)
    @popular_games_daily = Game.joins(:view_counts).where(view_counts: { created_at: 1.day.ago..Time.current }).group(:id).order('count(view_counts.id) desc').page(params[:popular_games_daily_page]).per(1)
